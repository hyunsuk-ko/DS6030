...
mean((linear_prediction - linear_train_yhat)^2)
```
...
# 
# result = drop_na(result)
# result = result %>% arrange(train_MSE)
# result
```
...
```{r}
# Train MSE
train_rmse = tibble(true_price = train$SalePrice, linear_yhat = linear_train_yhat, 
                    knn_yhat = knn_train_yhat, rf_yhat = rf_train_yhat, xgb_yhat = xgb_train_yhat, yhat = (linear_yhat + knn_yhat + rf_train_yhat + xgb_train_yhat) / 4, residual = true_price - yhat) 
train_rmse
```
...
my_prediction = test %>% select(Id, linear_yhat, knn_yhat, rf_yhat, xgb_yhat) %>% mutate(SalePrice = exp((linear_yhat + rf_yhat + xgb_yhat) / 3)) %>% select(-linear_yhat) %>% select(-knn_yhat) %>% select(-rf_yhat) %>% select(-xgb_yhat)
